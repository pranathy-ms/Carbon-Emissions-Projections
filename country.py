...
checkpoint_dir = 'sector_models'
os.makedirs(checkpoint_dir, exist_ok=True)

# Filter for only Afghanistan and Albania
#df = df[df['Entity'].isin(['Afghanistan', 'Albania'])]
...
for sector in tqdm(sector_columns, desc="Processing sectors"):
    ...
    for entity in tqdm(df['Entity'].unique(), desc="Processing entities", leave=False):
        ...
        #print(f"Stored predictions for {entity}, {sector}. Shape: {predictions_df.shape}")

    total_rolling_mse.append({'Sector': sector, 'mse': np.mean(sector_rolling_mse)})

    # Calculate global predictions for this sector
    matching_preds = [
        pred for entity_preds in all_predictions.values() 
        if sector in entity_preds
        for pred in [entity_preds[sector]]
    ]
    
    if matching_preds:
        sector_predictions = pd.concat(matching_preds)
        global_predictions_for_sector = sector_predictions.groupby('Year')['Predicted Emissions'].sum().reset_index()
    else:
        print(f"Warning: No predictions found for sector '{sector}'. This should not happen.")
        global_predictions_for_sector = pd.DataFrame(columns=['Year', 'Predicted Emissions'])
    ...
    model_path = os.path.join(checkpoint_dir, f'{sector}_model.h5')
    model.save(model_path)
    print(f"Model for {sector} saved to {model_path}")
...
for entity, sector_data in all_predictions.items():
    all_years = set()
    for pred in sector_data.values():
        all_years.update(pred['Year'].tolist())
    
    for year in sorted(all_years):
        row = {'Entity': entity, 'Year': year}
        for sector, predictions in sector_data.items():
            year_pred = predictions[predictions['Year'] == year]
            if not year_pred.empty:
                row[sector] = year_pred['Predicted Emissions'].values[0]
        final_df_rows.append(row)

final_df = pd.DataFrame(final_df_rows)

# Add the 'Code' column and reorder columns
#final_df['Code'] = 'N/A'
column_order = ['Entity', 'Year'] + sector_columns
